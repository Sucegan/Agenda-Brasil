...
function AppointmentItem({ item, role, onStatusChange, onConfirm, onCancel, onInformPayment, onPaymentStatusChange, pixKey, pixOwner }: {
  item: Appointment;
  role: 'cliente' | 'barbeiro' | 'admin';
  onStatusChange?: (status: AppointmentStatus) => void;
  onConfirm?: () => void;
  onCancel?: () => void;
  onInformPayment?: () => void;
  onPaymentStatusChange?: (status: Exclude<PaymentStatus, 'nao_exigido'>) => void;
  pixKey?: string | null;
  pixOwner?: string | null;
}) {
  const isProfessional = role === 'barbeiro' || role === 'admin';
  ...
  return (
    <article className="flex flex-col justify-between gap-4 rounded-xl border border-zinc-800/80 bg-zinc-950/45 p-4 sm:flex-row sm:items-center">
      <div className="min-w-0">
        ...
        <p className="font-bold text-zinc-100">{isProfessional ? item.cliente_nome : item.servico_nome}</p>
        <p className="mt-1 text-xs text-zinc-400">
          {role === 'admin'
            ? <>Profissional: {item.barbeiro_nome} · {item.servico_nome} · {formatCurrency(Number(item.servico_preco ?? 0))}</>
            : role === 'barbeiro'
              ? <>{item.servico_nome} · {formatCurrency(Number(item.servico_preco ?? 0))}</>
      ...
      <div className="flex flex-wrap items-center gap-2 sm:justify-end">
        ...
        {isProfessional && onStatusChange && <select value={item.status} onChange={(event) => onStatusChange(event.target.value as AppointmentStatus)} className="rounded-lg border border-zinc-700 bg-zinc-900 px-2.5 py-2 text-xs font-bold text-zinc-200 outline-none focus:border-emerald-500">
...
export default function DashboardPage() {
  ...
  const [activeArea, setActiveArea] = useState<DashboardArea>('inicio');
  ...
  const carregarDados = useCallback(async (forcedBusinessId?: string) => {
    ...
    if (perfil.tipo === 'cliente') {
      ...
      const selectedBusiness = publicBusinesses?.find((item) => item.id === (forcedBusinessId ?? barbeariaAtivaId)) ?? publicBusinesses?.[0];
  ...
  const barberIdsKey = useMemo(() => barbeiros.map((item) => item.id).sort((a, b) => a - b).join(','), [barbeiros]);
  ...
  useEffect(() => {
    if (tipoUsuario === 'admin' || tipoUsuario === 'proprietario') {
      ...
      for (const id of barberIdsKey.split(',')) {
        channel.on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'agendamentos', filter: `barbeiro_id=eq.${id}` },
    ...
    return () => { void supabase.removeChannel(channel); };
  }, [barbeariaAtivaId, barbeiroId, barberIdsKey, carregarDados, clienteId, tipoUsuario, usuarioId]);
  ...
  const agendar = async () => {
    ...
    const { data: appointment, error } = await supabase.rpc('criar_agendamento_com_origem', { p_barbeiro_id: selectedBarbeiro.id, p_servico_id: selectedServico.id, p_data: selectedData, p_horario: `${selectedHorario}:00`, p_origem: 'painel' });
  ...
  const confirmarAgendamento = async (id: number) => {
    ...
    await requestNotificationDelivery(id);
    await carregarDados();
    toast.success('Horário confirmado!', { id: toastId });
  };

  const cancelarAgendamento = async (id: number) => {
    const policy = negocio?.cancelamento_horas ? ` Cancelamentos com menos de ${negocio.cancelamento_horas}h serão registrados como tardios.` : '';
    if (!window.confirm(`Deseja cancelar este agendamento?${policy}`)) return;
  ...
  const informarPagamento = async (id: number) => {
    const { error } = await supabase.rpc('informar_pagamento_sinal', { p_agendamento_id: id });
    if (error) return toast.error(error.message);
    await carregarDados();
    toast.success('Pagamento informado. O profissional fará a confirmação.');
  };

  const atualizarPagamento = async (id: number, status: Exclude<PaymentStatus, 'nao_exigido'>) => {
    const { error } = await supabase.rpc('atualizar_sinal_agendamento', { p_agendamento_id: id, p_status: status });
    if (error) return toast.error(error.message);
    await carregarDados();
    toast.success('Situação do sinal atualizada.');
  };
  ...
  const criarConvite = async () => {
    ...
    const copied = await copyText(`${window.location.origin}/cadastro/profissional?convite=${data}`);
  ...
  const dashboardAreas: Array<{ id: DashboardArea; label: string; icon: typeof CalendarDays }> = usuario.tipo === 'cliente'
    ? [
        { id: 'inicio', label: 'Início', icon: CalendarCheck2 },
        { id: 'agendar', label: 'Novo horário', icon: CalendarDays },
        { id: 'historico', label: 'Histórico', icon: Clock },
        { id: 'avaliacoes', label: 'Avaliações', icon: Award },
      ]
    : [
        { id: 'inicio', label: isAdmin ? 'Administração' : 'Visão geral', icon: ShieldCheck },
        { id: 'agenda', label: 'Agenda', icon: CalendarDays },
        ...(isManager ? [{ id: 'equipe' as const, label: 'Equipe', icon: Users }] : []),
        { id: 'servicos', label: 'Serviços', icon: Scissors },
        { id: 'horarios', label: 'Horários', icon: Clock },
        { id: 'financeiro', label: 'Financeiro', icon: Wallet },
        { id: 'avaliacoes', label: 'Avaliações', icon: Award },
        ...(isManager ? [{ id: 'configuracoes' as const, label: 'Configurações', icon: Settings }] : []),
      ];
  ...
  return (
    <main className="app-screen app-shell safe-page-bottom text-zinc-100" style={brandStyle}>
      ...
      <section className="mx-auto max-w-7xl space-y-5 p-4 sm:p-6">
        ...
        <nav aria-label="Áreas do painel" className="sticky top-[76px] z-20 -mx-1 overflow-x-auto rounded-2xl border border-zinc-800 bg-zinc-950/95 p-2 shadow-2xl backdrop-blur-xl">
          <div className="flex min-w-max gap-2">
            {dashboardAreas.map(({ id, label, icon: Icon }) => (
              <button
                key={id}
                type="button"
                onClick={() => setActiveArea(id)}
                aria-current={activeArea === id ? 'page' : undefined}
                className={`flex min-h-11 items-center gap-2 rounded-xl border px-3 py-2 text-xs font-black transition-colors sm:px-4 ${activeArea === id ? 'border-emerald-400/60 bg-emerald-500/15 text-emerald-200' : 'border-transparent text-zinc-400 hover:border-zinc-700 hover:bg-zinc-900 hover:text-white'}`}
              >
                <Icon size={16} /> {label}
              </button>
            ))}
        ...
        {isOwner && (activeArea === 'inicio' || activeArea === 'financeiro') && <PlatformSubscription />}
        ...
        {perfilAberto && (
          <section className="rounded-2xl border border-emerald-500/25 bg-zinc-900/80 p-5 shadow-xl">
            ...
            <CommunicationPreferences user={usuario} appointments={agendamentos} onUpdated={carregarDados} />
          </section>
        )}

        {isBusinessOwner && negocio && !barbeiro && ['inicio', 'equipe', 'configuracoes', 'financeiro'].includes(activeArea) && (
          <div className="space-y-5">
            {(activeArea === 'inicio' || activeArea === 'equipe') && <section className="panel-card border-amber-500/25 p-5"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><h2 className="font-black text-white">Monte a equipe desta unidade</h2><p className="mt-1 text-sm text-zinc-500">Nenhum profissional está vinculado. Envie um convite para liberar serviços e agenda.</p></div><button onClick={() => { void criarConvite(); }} className="primary-button"><Copy size={15} /> Copiar convite de profissional</button></div></section>}
            {activeArea === 'configuracoes' && <section className="panel-card p-5"><h2 className="mb-4 flex items-center gap-2 text-lg font-black"><Settings className="text-emerald-400" size={20} /> Dados do estabelecimento</h2><form onSubmit={salvarNegocio} className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-bold text-zinc-400">NOME<input value={nomeNegocio} onChange={(event) => setNomeNegocio(event.target.value)} className="mt-1.5 w-full rounded-xl border border-zinc-800 bg-zinc-950 p-3" /></label><label className="text-xs font-bold text-zinc-400">TELEFONE<input value={telefoneNegocio} onChange={(event) => setTelefoneNegocio(event.target.value)} className="mt-1.5 w-full rounded-xl border border-zinc-800 bg-zinc-950 p-3" /></label><label className="text-xs font-bold text-zinc-400">ENDEREÇO<input value={enderecoNegocio} onChange={(event) => setEnderecoNegocio(event.target.value)} className="mt-1.5 w-full rounded-xl border border-zinc-800 bg-zinc-950 p-3" /></label><label className="text-xs font-bold text-zinc-400">URL DA LOGO<input type="url" value={logoNegocio} onChange={(event) => setLogoNegocio(event.target.value)} className="mt-1.5 w-full rounded-xl border border-zinc-800 bg-zinc-950 p-3" /></label><button className="primary-button sm:col-span-2">Salvar dados</button></form></section>}
            {activeArea === 'configuracoes' && <BusinessGrowthSettings business={negocio} onUpdated={carregarDados} />}
            {activeArea === 'financeiro' && <FinancialCenter barbershopId={negocio.id} />}
          </div>
        )}
        {isBusinessOwner && negocio && !barbeiro && ['agenda', 'servicos', 'horarios', 'avaliacoes'].includes(activeArea) && (
          <section className="panel-card border-amber-500/25 p-6 text-center"><Users className="mx-auto text-amber-300" size={30} /><h2 className="mt-3 text-lg font-black">Adicione um profissional primeiro</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-zinc-500">Serviços, horários, agenda e avaliações dependem de um profissional vinculado a esta unidade.</p><button type="button" onClick={() => setActiveArea('equipe')} className="primary-button mt-5">Ir para Equipe</button></section>
        )}

        {usuario?.tipo === 'cliente' && (
          <div className="space-y-5">
            {activeArea === 'inicio' && negocio && <NotificationInbox userId={usuario.id} barbershopId={negocio.id} />}
            {activeArea === 'agendar' && <section className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/70 p-5 shadow-xl sm:p-6">
            ...
            {activeArea === 'avaliacoes' && <ReviewLoyalty role="cliente" appointments={agendamentos} points={clientPoints} />}
          </div>
        )}
