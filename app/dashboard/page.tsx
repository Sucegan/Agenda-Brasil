...
import { 
  ...
  LogOut, Plus, Trash2, CheckCircle2, XCircle, MessageCircle, ChevronDown
} from 'lucide-react';

// ==========================================
// COMPONENTE 1: SKELETON
// ==========================================
const DashboardSkeleton = () => (
  <main className="min-h-screen bg-zinc-950 p-6 flex flex-col gap-6 animate-pulse w-full items-center">
    ...
    <div className="h-[200px] bg-zinc-900 rounded-2xl max-w-4xl w-full border border-zinc-800 grid grid-cols-3 gap-4"></div>
    <div className="h-[400px] bg-zinc-900 rounded-xl max-w-4xl w-full border border-zinc-800"></div>
  </main>
);

// ==========================================
// COMPONENTE 2: DROPDOWN CUSTOMIZADO (STATUS)
// ==========================================
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold tracking-wider border transition-all hover:brightness-125 ${current.color}`}
      >
        {current.label} <ChevronDown size={14} className={`transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>

      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-40 rounded-xl border border-zinc-700 bg-zinc-800 shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
          {Object.entries(statusConfig).map(([key, config]) => (
            <button
              key={key}
              onClick={() => { onChange(key); setIsOpen(false); }}
              className="w-full text-left px-4 py-3 text-xs font-bold tracking-wider hover:bg-zinc-700 transition-colors border-b border-zinc-700/50 last:border-0"
            >
  ...
  return (
    <main className="min-h-screen bg-zinc-950 p-6 text-zinc-100 font-sans relative pb-20 selection:bg-emerald-500/30">
      ...
      <header className="mx-auto flex max-w-4xl items-center justify-between border-b border-zinc-800/80 pb-6 mb-8">
        ...
        <button onClick={() => { supabase.auth.signOut(); router.push('/'); }} className="flex items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-2 text-sm font-medium text-zinc-300 hover:bg-zinc-800 hover:text-white transition-all">
          <LogOut size={16} /> Sair
        </button>
      </header>
      ...
      <section className="mx-auto max-w-4xl space-y-6">
        ...
        {usuario?.tipo === 'cliente' && (
           <div className="space-y-6">
              {/* CÓDIGO DO CLIENTE MANTIDO IGUAL AO ANTERIOR, MAS ESTILIZADO... */}
              <div className="rounded-2xl border border-zinc-800/80 bg-zinc-900/50 backdrop-blur-xl p-6 shadow-xl relative overflow-hidden">
                {/* Lógica do Step Wizard mantida */}
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-xl font-bold flex items-center gap-2"><CalendarDays className="text-emerald-500" /> Novo Agendamento</h2>
                  {step > 1 && <button onClick={() => {setStep(1); setSelectedBarbeiro(null); setSelectedServico(null); setSelectedData('');}} className="text-xs text-zinc-400 hover:text-white transition">↺ Reiniciar</button>}
                </div>
                {/* Steps 1, 2, 3... */}
                {step === 1 && (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {barbeiros.map((b: any) => (
                        <button key={b.id} onClick={() => { setSelectedBarbeiro(b); setStep(2); }} className="flex items-center gap-4 p-4 rounded-xl border border-zinc-800 bg-zinc-800/30 hover:border-emerald-500/50 hover:bg-zinc-800 transition-all text-left group">
                          <div className="h-12 w-12 rounded-full bg-zinc-700 flex items-center justify-center text-xl font-bold text-emerald-400 group-hover:bg-emerald-500 group-hover:text-zinc-900 transition-colors">{b.nome?.charAt(0).toUpperCase()}</div>
                          <div><p className="font-semibold">{b.nome}</p><p className="text-xs text-zinc-500">{b.dias_trabalho}</p></div>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="animate-in fade-in duration-300">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {servicos.filter(s => s.barbeiro_id === selectedBarbeiro.id).map((s: any) => (
                        <button key={s.id} onClick={() => { setSelectedServico(s); setStep(3); }} className="flex flex-col items-center p-4 rounded-xl border border-amber-600/20 bg-amber-500/5 hover:bg-amber-500 hover:text-zinc-950 transition-all text-center group text-amber-500">
                          <span className="font-semibold text-sm group-hover:text-zinc-950">{s.nome}</span>
                          <span className="font-bold mt-1 group-hover:text-zinc-900">R$ {Number(s.preco).toFixed(2)}</span>
                          <span className="text-[10px] opacity-70 mt-1">({s.duracao} min)</span>
                        </button>
                      ))}
                    </div>
                  </div>
                    ...
                    {selectedData && (
                      <div className="mt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        {horariosDisponiveis.length > 0 ? (
                          <>
                            <p className="mb-3 text-sm font-medium text-zinc-400">Selecione o horário:</p>
                            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 mb-6">
                              {horariosDisponiveis.map((hora: string) => (
                                <button key={hora} onClick={() => setSelectedHorario(hora)} className={`py-2 rounded-lg text-sm font-bold border transition-all ${selectedHorario === hora ? 'bg-emerald-500 border-emerald-400 text-zinc-950 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'bg-zinc-800 border-zinc-700 text-zinc-300 hover:bg-zinc-700'}`}>{hora}</button>
                              ))}
                            </div>
                          ...
                          <div className="rounded-xl border border-red-900/50 bg-red-950/20 p-4 text-center"><p className="text-sm font-medium text-red-400">🕒 Sem horários disponíveis.</p></div>
                        )}
                      </div>
                    )}
                  </div>
        ...
        {/* ==========================================
            ÁREA DO BARBEIRO (DESIGN PREMIUM)
        ========================================== */}
        {usuario?.tipo === 'barbeiro' && (
          <>
            {/* Cards Financeiros Refinados */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
               <div className="bg-zinc-900/60 backdrop-blur-sm border border-zinc-800/80 p-6 rounded-2xl shadow-xl relative overflow-hidden group hover:border-emerald-500/50 transition-colors">
                 <div className="absolute top-4 right-4 text-emerald-500/10 group-hover:text-emerald-500/20 transition-colors"><Wallet size={64} /></div>
                 <p className="text-zinc-400 text-[11px] font-bold uppercase tracking-widest mb-2">Faturamento Hoje</p>
                 <p className="text-3xl font-black text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.3)]">R$ {faturamentoHoje.toFixed(2)}</p>
               </div>
               <div className="bg-zinc-900/60 backdrop-blur-sm border border-zinc-800/80 p-6 rounded-2xl shadow-xl relative overflow-hidden group hover:border-zinc-600 transition-colors">
                 <div className="absolute top-4 right-4 text-zinc-500/10 group-hover:text-zinc-500/20 transition-colors"><Scissors size={64} /></div>
               ...
               <div className="bg-zinc-900/60 backdrop-blur-sm border border-zinc-800/80 p-6 rounded-2xl shadow-xl relative overflow-hidden group hover:border-amber-500/50 transition-colors">
                 <div className="absolute top-4 right-4 text-amber-500/10 group-hover:text-amber-500/20 transition-colors"><TrendingUp size={64} /></div>
                 <p className="text-zinc-400 text-[11px] font-bold uppercase tracking-widest mb-2">Previsão Futura</p>
                 <p className="text-3xl font-black text-amber-500 drop-shadow-[0_0_8px_rgba(245,158,11,0.3)]">R$ {previsaoSemana.toFixed(2)}</p>
               </div>
            </div>

            {/* Expediente com Inputs Refinados */}
                ...
                {[{label: 'Início', val: horarioInicio, set: setHorarioInicio, color: 'focus:border-emerald-500'}, 
                  ...
                  <div key={i} className="relative group">
                    ...
                    <input type="time" value={campo.val} onChange={e => campo.set(e.target.value)} 
                           className={`time-input w-full rounded-xl border border-zinc-800 bg-zinc-950/50 p-3 text-white outline-none transition-colors ${campo.color} hover:border-zinc-600`} />
                  </div>
                ))}
                <div className="col-span-2 md:col-span-1">
                  <button type="submit" className="w-full rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold p-3 transition-colors flex items-center justify-center gap-2">
            ...
            <div className="rounded-2xl border border-zinc-800/80 bg-zinc-900/60 backdrop-blur-sm p-6 shadow-xl">
              <h2 className="mb-6 text-lg font-bold flex items-center gap-2"><CalendarDays size={20} /> Minha Agenda</h2>
              {agendamentos.length === 0 ? <p className="text-sm text-zinc-500 flex items-center gap-2"><CalendarDays size={16} /> Nenhum cliente agendado.</p> : (
                <div className="space-y-3">
                  {agendamentos.map((item: any) => (
                    <div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between rounded-xl border border-zinc-800/60 bg-zinc-950/30 p-5 gap-4 hover:border-zinc-700 transition-colors">
                      <div>
                        <div className="flex items-center gap-4 mb-1">
                           ...
                           {item.clientes?.telefone && (
                             <a href={`${formatarZap(item.clientes.telefone)}?text=Olá ${item.clientes.nome}, confirmando seu agendamento para hoje às ${item.horario?.slice(0, 5)}!`} target="_blank" rel="noopener noreferrer" 
                                className="bg-[#25D366]/10 text-[#25D366] text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 hover:bg-[#25D366]/20 transition-colors border border-[#25D366]/20">
                               <MessageCircle size={14} /> WhatsApp
                             </a>
                           )}
                        </div>
                        <p className="text-sm text-zinc-300 font-medium">{item.clientes?.nome} <span className="text-xs text-zinc-600 ml-2 font-mono">{item.clientes?.telefone}</span></p>
                        <p className="text-xs text-zinc-500 mt-1.5 flex items-center gap-1.5"><Scissors size={12} /> {item.servicos?.nome} • R$ {Number(Array.isArray(item.servicos) ? item.servicos[0]?.preco : item.servicos?.preco).toFixed(2)}</p>
                      </div>
                      
                      <div className="flex flex-col items-end justify-between gap-3">
                        {/* AQUI ESTÁ O NOVO DROPDOWN! ADEUS SELECT AZUL FEIO */}
                        <StatusDropdown currentStatus={item.status} onChange={(novoStatus) => atualizarStatus(item.id, novoStatus)} />
                        
                        <p className="text-[11px] text-zinc-500 font-bold uppercase tracking-widest bg-zinc-900 px-2 py-1 rounded-md border border-zinc-800">
            ...
            <div className="rounded-2xl border border-zinc-800/80 bg-zinc-900/60 backdrop-blur-sm p-6 shadow-xl">
              ...
              <form onSubmit={handleAdicionarServico} className="grid grid-cols-1 gap-4 md:grid-cols-4 mb-8 bg-zinc-950/40 p-5 rounded-xl border border-zinc-800/50">
                <div className="md:col-span-2"><label className="mb-1.5 block text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Nome do Serviço</label><input type="text" value={novoServicoNome} onChange={e => setNovoServicoNome(e.target.value)} className="w-full rounded-xl border border-zinc-800 bg-zinc-900 p-3 text-white outline-none focus:border-amber-500 transition-colors" /></div>
                <div><label className="mb-1.5 block text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Preço</label><input type="text" value={novoServicoPrecoFormatado} onChange={handlePrecoChange} placeholder="R$ 0,00" className="w-full rounded-xl border border-zinc-800 bg-zinc-900 p-3 text-white outline-none focus:border-amber-500 transition-colors" /></div>
                <div><label className="mb-1.5 block text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Duração (Min)</label><input type="number" min="1" value={novoServicoDuracao} onChange={e => setNovoServicoDuracao(e.target.value)} className="w-full rounded-xl border border-zinc-800 bg-zinc-900 p-3 text-white outline-none focus:border-amber-500 transition-colors" /></div>
                <div className="md:col-span-4 flex justify-end"><button type="submit" className="rounded-xl bg-amber-600 hover:bg-amber-500 text-zinc-950 font-bold px-6 py-3 transition-colors flex items-center gap-2"><Plus size={18} /> Adicionar Serviço</button></div>
              </form>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {servicos.map((s: any) => (
                  <div key={s.id} className="flex items-center justify-between rounded-xl border border-zinc-800/60 bg-zinc-950/30 p-4 hover:border-zinc-700 transition-colors">
                    <div>
                      <p className="font-bold text-white text-sm">{s.nome}</p>
                      <p className="text-xs text-zinc-500 mt-0.5 flex items-center gap-1"><Clock size={12} /> {s.duracao} min</p>
                      <p className="font-black text-amber-500 mt-2">R$ {Number(s.preco).toFixed(2)}</p>
                    </div>
                    <button onClick={() => handleExcluirServico(s.id)} className="rounded-lg bg-red-500/10 px-3 py-2 text-xs font-bold text-red-400 hover:bg-red-500/20 border border-red-500/20 transition-colors flex items-center gap-2"><Trash2 size={14} /> Excluir</button>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* ESTILOS GLOBAIS PARA FORÇAR O RELÓGIO A FICAR BONITO */}
      <style jsx global>{`
        .hide-scrollbar::-webkit-scrollbar { display: none; } 
        .hide-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
        
        /* Mágica para o Input de Tempo nativo ficar invisível/integrado no Dark Mode */
