...
export default function PublicBookingPage() {
  ...
  const [sessionAccountType, setSessionAccountType] = useState<AccountType | null>(null);
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      if (userId) {
        const { data: profile, error: profileError } = await supabase
          ...
          .eq('id', userId)
          .maybeSingle();
        if (profileError) toast.error('Não foi possível identificar o tipo da sua conta.');
        accountType = profile?.tipo ?? null;
      }
      if (!active) return;
      setSessionUserId(userId);
      setSessionAccountType(accountType);
      setLoading(false);

      const search = new URLSearchParams(window.location.search);
      const intentToken = search.get('intent');
      if (userId && accountType === 'cliente' && intentToken) {
        await completeIntent(intentToken);
      } else if (userId && accountType === 'cliente' && search.get('retomar') === '1') {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      setSessionUserId(userId);
      setSessionAccountType(null);
      // Supabase invokes this callback while holding its auth lock. Schedule
      // profile lookup and follow-up outside the callback so auth calls cannot
      // deadlock, which was especially visible in Safari.
      if (userId) {
        window.setTimeout(() => {
          void (async () => {
            const { data: profile } = await supabase.from('usuarios').select('tipo').eq('id', userId).maybeSingle();
            const accountType = profile?.tipo ?? null;
            setSessionAccountType(accountType);
            const intentToken = new URLSearchParams(window.location.search).get('intent');
            if (accountType === 'cliente' && intentToken) await completeIntent(intentToken);
          })();
        }, 0);
      }
    });
  ...
  const continueAsClient = async () => {
    ...
    toast.success('Agora informe os dados do cliente para continuar.');
  };
      ...
      <section className="mx-auto max-w-5xl space-y-5 p-4 sm:p-6">
        ...
        {date && (time || slots.length === 0) && (
          <section className="rounded-2xl border border-emerald-500/25 bg-zinc-900/80 p-5 shadow-xl">
            <h2 className="mb-4 flex items-center gap-2 font-black"><UserRound className="text-emerald-400" size={20} /> 4. Identificação segura</h2>
            {isAuthenticatedClient ? (
              <p className="mb-4 flex items-center gap-2 text-sm text-emerald-300"><ShieldCheck size={17} /> Você está identificado como cliente.</p>
            ) : isProfessionalSession ? (
              <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-100">
                <p className="font-bold">Você está conectado com uma conta profissional.</p>
                <p className="mt-1 text-amber-200/80">Contas profissionais administram a agenda, mas não podem reservar como cliente.</p>
                <button onClick={() => { void continueAsClient(); }} className="mt-3 rounded-lg bg-amber-400 px-3 py-2 text-xs font-black text-zinc-950 hover:bg-amber-300">Sair e continuar como cliente</button>
              </div>
            ) : (
