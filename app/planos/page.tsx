    ...
    <main className="app-screen bg-zinc-950 px-4 py-8 text-zinc-100 sm:px-6 sm:py-12">
      <section className="mx-auto max-w-6xl">
        ...
        <header className="mx-auto max-w-3xl py-14 text-center sm:py-20">
          ...
          <Link href="/?tipo=proprietario" className="mt-7 inline-flex items-center gap-2 rounded-2xl bg-emerald-600 px-6 py-4 font-black text-white shadow-xl shadow-emerald-950/40 hover:bg-emerald-500"><BadgeCheck size={19} /> Começar teste grátis</Link>
        </header>
        ...
        <section className="grid gap-5 lg:grid-cols-3" aria-label="Planos da plataforma">
          {(plans.length ? plans : Array.from({ length: 3 }, (_, index) => ({ id: index } as PlatformPlanPublic))).map((plan) => plan.nome ? (
            <article key={plan.id} className={`relative flex flex-col rounded-3xl border p-6 shadow-2xl ${plan.destaque ? 'border-emerald-400/60 bg-emerald-500/10' : 'border-zinc-800 bg-zinc-900/70'}`}>
              ...
              <Link href="/?tipo=proprietario" className={`flex items-center justify-center rounded-xl px-4 py-3 text-sm font-black ${plan.destaque ? 'bg-emerald-500 text-zinc-950 hover:bg-emerald-400' : 'border border-zinc-700 bg-zinc-950 text-white hover:border-emerald-500/50'}`}>Testar {plan.nome}</Link>
            </article>
