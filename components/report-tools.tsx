...
export function ReportTools({ appointments }: { appointments: Appointment[] }) {
  ...
  const clients = completed.reduce<Record<string, number>>((acc, item) => {
    const key = item.cliente_telefone || item.cliente_nome || `cliente-${item.cliente_id}`;
    acc[key] = (acc[key] ?? 0) + 1;
    return acc;
  }, {});
  const clientCounts = Object.values(clients);
  const repeatRate = clientCounts.length ? Math.round(clientCounts.filter((count) => count > 1).length / clientCounts.length * 100) : 0;
  const revenueByHour = completed.reduce<Record<string, { revenue: number; count: number }>>((acc, item) => {
    const hour = `${item.horario.slice(0, 2)}h`;
    const current = acc[hour] ?? { revenue: 0, count: 0 };
    acc[hour] = { revenue: current.revenue + Number(item.servico_preco ?? 0), count: current.count + 1 };
    return acc;
  }, {});
  const bestHour = Object.entries(revenueByHour).sort(([, a], [, b]) => b.revenue - a.revenue)[0];
