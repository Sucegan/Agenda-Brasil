...
export function ReviewLoyalty({ role, appointments, points = 0, barberId, barberIds }: { role: 'cliente' | 'barbeiro' | 'admin'; appointments: Appointment[]; points?: number; barberId?: number; barberIds?: number[] }) {
  ...
  const load = useCallback(async () => {
    ...
    if (role === 'admin' && barberIdsKey) query = query.in('barbeiro_id', barberIdsKey.split(',').map(Number));
  ...
  if (role === 'barbeiro' || role === 'admin') return <section className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-5 shadow-xl"><div className="flex items-start justify-between gap-4"><div><h2 className="flex items-center gap-2 text-lg font-black"><MessageSquareText className="text-amber-400" size={20} /> Avaliações</h2><p className="mt-1 text-xs text-zinc-500">{role === 'admin' ? 'Feedback privado de toda a unidade.' : 'Feedback privado dos seus clientes.'}</p></div><div className="text-right"><b className="text-2xl text-amber-300">{reviews.length ? average.toFixed(1) : '—'}</b><p className="text-[10px] text-zinc-500">{reviews.length} avaliação(ões)</p></div></div><div className="mt-4 space-y-2">{reviews.slice(0, 5).map((review) => <article key={review.id} className="rounded-xl border border-zinc-800 bg-zinc-950/50 p-3"><div className="flex gap-0.5" aria-label={`${review.nota} de 5 estrelas`}>{Array.from({ length: 5 }, (_, index) => <Star key={index} size={14} className={index < review.nota ? 'fill-amber-400 text-amber-400' : 'text-zinc-700'} />)}</div>{review.comentario && <p className="mt-2 text-sm text-zinc-300">{review.comentario}</p>}</article>)}{!reviews.length && <p className="text-sm text-zinc-500">As primeiras avaliações aparecerão aqui.</p>}</div></section>;
