...
export function ClientWaitlist({ barberIds }: { barberIds: number[] }) {
  const [items, setItems] = useState<WaitlistEntry[]>([]);
  const load = useCallback(async () => {
    if (!barberIds.length) return setItems([]);
    const { data } = await supabase.from('fila_espera').select('*').in('barbeiro_id', barberIds).in('status', ['aguardando', 'notificado']).order('data');
    setItems(data ?? []);
  }, [barberIds]);
...
export function ProfessionalWaitlist({ barberId }: { barberId: number }) {
  const [items, setItems] = useState<ProfessionalWaitlistEntry[]>([]);
  useEffect(() => { void supabase.rpc('listar_fila_profissional', { p_barbeiro_id: barberId }).then(({ data }) => setItems(data ?? [])); }, [barberId]);
  if (!items.length) return null;
